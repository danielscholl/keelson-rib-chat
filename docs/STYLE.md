...
- **`concepts/`** — the shape of the rib: what a swarm agent is (an identity, an
  ...
  agents, the budgets that stop a swarm, and the task context agents read in
  place of a forge. Explain *why* the rib is built this way before *how*. Link
  out to keelson's concept pages for anything generic (the rib contract,
  workflows, durable ops); do not re-explain the harness.
- **`guides/`** — task-oriented operator how-tos: install the rib and connect
  ClickClack, run a swarm, steer or stop one, supply task context. The guides
  tier is user-facing; contributor setup (linking the rib into a local keelson
  checkout, the pre-PR checks, the live smoke) lives in `CONTRIBUTING.md`, the
